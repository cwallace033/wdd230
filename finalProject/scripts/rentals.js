document.addEventListener("DOMContentLoaded", () => {
    ...
    async function getLinksData(url) {
        try {
            ...
            if (response.ok) {
                ...
                if (data && data["Max Rental Pricing"]) {
    ...
    function populateTable(data) {
        for (const rentalType in data["Reservation"]) {
            const maxPersons = data["Reservation"][rentalType]["Max Riders"];
            const reservationHalfDay = data["Reservation"][rentalType]["Half Day (3 hrs)"];
            const reservationFullDay = data["Reservation"][rentalType]["Full Day"];
            const walkInHalfDay = data["Walk-In"][rentalType]["Half Day (3 hrs)"];
            const walkInFullDay = data["Walk-In"][rentalType]["Full Day"];
